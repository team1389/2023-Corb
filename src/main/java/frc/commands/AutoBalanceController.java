...
//lowest succesful value with new robot: 
    ...
    public void execute(){
        ...
        double driveAngle = Math.asin(Math.sin(roll)/(Math.sqrt((Math.pow(Math.sin(pitch),2))+(Math.pow(Math.sin(roll),2)))));
        double tempSpeed;
        ...
        // Calculate the control output using a PID controller
            ...
            ChassisSpeeds chassisSpeeds = new ChassisSpeeds(tempSpeed*Math.cos(driveAngle), tempSpeed*Math.sin(driveAngle), 0); 
